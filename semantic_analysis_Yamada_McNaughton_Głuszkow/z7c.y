...
RE: RE '|' RE {
  $$ = createRE(
    RE_NULL($1) || RE_NULL($3), // Null
    merge_sets(RE_FIRST($1), RE_FIRST($3)), // First
    merge_sets(RE_LAST($1), RE_LAST($3)), // Last
    merge_follow_sets(RE_FOLLOW($1), RE_FOLLOW($3)) // Follow
  );
};
